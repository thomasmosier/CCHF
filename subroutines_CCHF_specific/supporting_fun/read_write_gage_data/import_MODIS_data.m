...
%Find folder, filename, etc.
[foldNm, fileNm, ext] = fileparts(path);

%Create formatted name to save processed output:
indPer = regexpi(fileNm, '\.');
if ~isempty(indPer)
    fileOut = ['CCHF_processed_' fileNm(1:indPer(1)-1) fileNm(indPer(2)+1:end) '_' strType '.mat'];
else
    fileOut = ['CCHF_processed_' fileNm '_' strType '.mat']; 
end
pathOut = fullfile(foldNm, fileOut);


if exist(pathOut, 'file') %Load if processed MODIS images already exist
   load(pathOut, 'sGageCurr');
