...
function varargout = mass_enbal(varargin)
...
minSnowTmp = -50; %Minimum temperature snow can be (Deg C)
...
energS = sCryo.heat*time2sec(1,sMeta.dt,sMeta.currTime);

%Consider energy losses due to conduction into snowpack:
%See Regine Hock's "A Mass-Balance, Glacier Runoff and Multi-Layer Snow
%Model" program documentation and user manual, pg. 19.
%Also see:
%Sturm, M., Holmgren, J., König, M., & Morris, K. (1997). The thermal 
%conductivity of seasonal snow. Journal of Glaciology, 43(143), 26–41.


%density of fresh snow modelled in 'snow_accum'. Other density assumptions
%made here:
indF = find(~isnan(sCryo.densityF));
%BEST DENSIFICATION MODEL (USED IN CONDUCTION):
%Bormann, K. J., Westra, S., Evans, J. P., & McCabe, M. F. (2013). Spatial and temporal variability in seasonal snow density. Journal of Hydrology, 484, 63–73. http://doi.org/10.1016/j.jhydrol.2013.01.032
%Others:
% %Model from Bader, H. (1954). Sorge’s Law of densification of snow on high polar glaciers. J. Glaciol, 2(15), 319–323.
% sSnowpack.density = 0.338 ...
%     + 0.01958*sSnowpack.solid ...
%     + 0.00202274*(sSnowpack.solid).^2 ...
%     + 0.000329092*(sSnowpack.solid).^3 ...
%     + 0.0000115327*(sSnowpack.solid).^4;
% %Model for snow drifts from Pomeroy, J. W., & Brun, E. (2001). Physical properties of snow. Snow Ecology, 45–126.
% sSnowpack.density = 522 ...
%     - (20470./(1000*sSnowpack.solid)).*(1-exp(-(1000*sSnowpack.solid)/67.3));
%Modified Pomeroy and Brun
sCryo.density = (204700./(1000*sCryo.solid)).*(1-exp(-(1000*sCryo.solid)/67.3));

sCryo.density(indF) = sCryo.densityF(indF);
sCryo.density(sCryo.liquid > 0) = 700;

%ACCOUNT FOR ENERGY LOSSES FROM CONDUCTION AT SNOW SURFACE LAYER:
%Reference: Hock User Manual (http://www2.gi.alaska.edu/~regine/modelmanual.pdf)
sCryo.conduct = -0.871*10^(-2) + 0.439*10^(-3)*sCryo.density+1.05*10^(-6)*(sCryo.density).^2;
sCryo.conduct(sCryo.conduct > 1) = 1;
sCryo.conduct(isnan(sCryo.conduct)) = 1;

% sCryo.conduct

%Account for energy losses from conduction at snowsurface layer
energS = energS*sCryo.conduct;
...
sCryo.latentSnow(indReFreeze) = -cLate*1000*sCryo.liquid(indReFreeze);

indMaxRf = intersect(indReFreeze, find(sCryo.latentSnow < energS));
sCryo.latentSnow(indMaxRf) = energS(indMaxRf);

dSnowCurr = -sCryo.latentSnow(indReFreeze) ./ (cLate*1000);
...
dTempCurr = sCryo.sensibleSnow(indLowTemp) ./ (cSens*1000*sCryo.solid(indLowTemp));
    dTempCurr(isnan(dTempCurr)) = 0; %Occurs when numerator and denominator are NaN
...
sCryo.sensibleSnow(indRaisTemp) = -cSens*1000*sCryo.solid(indRaisTemp).*sCryo.tsn(indRaisTemp);
...
dTempCurr = sCryo.sensibleSnow(indRaisTemp) ./ (cSens*1000*sCryo.solid(indRaisTemp));
    dTempCurr(isnan(dTempCurr)) = 0; %Occurs when numerator and denominator are NaN
...
sCryo.latentSnow(indMelt) = cLate*1000*sCryo.solid(indMelt);
indMaxMelt = intersect(indMelt, find(sCryo.latentSnow > energS));
sCryo.latentSnow(indMaxMelt) = energS(indMaxMelt);

dSnowCurr = -sCryo.latentSnow(indMelt) ./ (cLate*1000);
...
energS(indMelt) = energS(indMelt) - sCryo.latentSnow(indMelt);

%Melt ice:
indMeltIce = intersect(intersect(indPos, find(sCryo.ice > 0)), energS > 0);
sCryo.latentIce(indMeltIce) = energS(indMeltIce) .* sCryo.heatI(indMeltIce) ./ sCryo.heat(indMeltIce);
sCryo.dIce(indMeltIce) = -sCryo.latentIce(indMeltIce) / (cLate*1000); 
...
% sCryo.tsn(sCryo.tsn < minSnowTmp) = minSnowTmp;
% sCryo.dTmp(sCryo.dTmp < minSnowTmp) = minSnowTmp;
sCryo.dTmp(sCryo.dTmp > abs(minSnowTmp)) = abs(minSnowTmp);
