...
for ii = 1 : numel(namesObs)
    ...
    namesCurr(strcmpi(namesCurr,'date')) = [];
    namesCurr(strcmpi(namesCurr,'dateStart')) = [];
    namesCurr(strcmpi(namesCurr,'dateEnd')) = [];
    namesCurr(strcmpi(namesCurr,'time')) = [];
    namesCurr(strcmpi(namesCurr,'attributes')) = [];
    namesCurr(strcmpi(namesCurr,'att')) = [];
        ...
        if ~isempty(fieldNotOutCurr)
            for kk = 1 : numel(fieldNotOutCurr)
                if isnumeric(lonCurr) && isnumeric(latCurr)
                    output(end+1,:) = {char(fieldNotOutCurr{kk}), [lonCurr, latCurr]};
                elseif strcmpi(lonCurr,'avg')
                    output(end+1,:) = {fieldNotOutCurr{kk}, [lonCurr, latCurr]};
                end
            end
        end
