...
%     end
    ...
    if ii == 1
        ...
        sInput.all.(varLd) = nan(numel(filesPlot),1);
    end
...
%         dataUnit = sDataCurr.attData{rUnit, cUnit+1};
    ...
    for jj = 1 : sum(~isnan(vecDays(ii,:)))
        ...
        if ii == 1
            ...
            sInput.all.(varLd) = nan(numel(vecDays(:)),1);
                sInput.all.lon = NaN;
                sInput.all.lat = NaN;
        end
        
        sInput.all.date(cntr,:) = days_2_date(vecDays(ii,indTime(jj)),dateRef,cal);
        sInput.all.(varLd)(cntr) = sum2d(areaGrid.*squeeze(sDataCurr.data(indTime(jj),:,:)));
    end
end 
close(hWait);


foldOut = fullfile(foldPlot,'analysis');
mkdir(foldOut);
