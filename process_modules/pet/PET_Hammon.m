...
for ii = 1 : numel(d)
    ...
    PET(ii,:,:) = (10^a)*0.001*18.2*N.*exp(17.27*squeeze(sAtm.tas(sAtm.indCurr,:,:))./(squeeze(sAtm.tas(sAtm.indCurr,:,:))+273.3)) ...
        ./(squeeze(sAtm.tas(sAtm.indCurr,:,:))+273.3); %Units of PET in above are m/day
%    PET(ii,:,:) = (10^a)*218.3946*N.*exp((17.27*squeeze(sAtm.tas(sAtm.indCurr,:,:)))./(squeeze(sAtm.tas(sAtm.indCurr,:,:))+273.3)) ...
%        ./((squeeze(sAtm.tas(sAtm.indCurr,:,:))+273.3)*1000); %Units of PET in above are m/day
end
