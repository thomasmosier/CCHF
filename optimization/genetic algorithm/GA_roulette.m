function newCoef = GA_roulette(oldCoef, error, sOpt)
%Each row is different coefficient set
...
for ii = 1 : nSelect
    ...
    newCoef(ii,:) = oldCoef(indSel,:);
end
 
if ~isempty(nElite) && nElite > 0
    newCoef(nSelect+1:nSelect + nElite,:) = oldCoef(1:nElite,:);
end
